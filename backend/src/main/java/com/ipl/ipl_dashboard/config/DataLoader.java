...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // If data already exists (e.g., using a persistent DB in production),
        // skip CSV processing to avoid slow cold starts and duplicate loads.
        if (teamRepo.count() > 0 && matchRepo.count() > 0 && playerRepo.count() > 0) {
            System.out.println("⏭️  Data already present in DB. Skipping CSV load.");
            return;
        }
